...
@section('content')
    ...
    <div class="container">
        <div class="card">
            <div class="card-body">
                ...
                <form id="form" action="{{ route('products.update',$product->id) }}" method="post" class="mt-4">
                    @csrf @method('PUT')
                    ...
                    <div class="more_ingredient_area">
                        @if($k != 0)
                            <span class="btn btn-danger btn-sm ml-2 remove" id="{{ $k }}">&times;</span>
                        @endif
                        <h5>Ingredient for 1 person</h5>

                        <div class="form-group">
                            <label for="ingredient">Ingredient</label>
                            <select name="ingredient[]" id="ingredient" used="{{ $ingredient_in->ingredient_id }}" class="form-control ingredients used u_s_{{ $k }}" required>
                                <option value="">choose one</option>
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control" min="1" value="{{ $ingredient_in->quantity }}" name="quantity[]" id="quantity" placeholder="Enter quantity for 1 person" required>
                        </div>
                    </div>
                    @endforeach
    ...
    <script>
        $(document).ready(function () {
            var i = {{ count($ingredient_in_product) }};
            ...
            @foreach($ingredient_in_product as $k => $ingredient_in)
                used_ingredients.push('{{ $ingredient_in->ingredient_id }}');
            @endforeach
